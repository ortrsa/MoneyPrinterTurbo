...
8c. **Hashtags follow the 3-tier tag formula — 9-12 tags total, never just 2-3.** For `platform="youtube_shorts"`, `generate_social_metadata` returns exactly 12 hashtags (`SOCIAL_PLATFORMS["youtube_shorts"]["hashtag_count"]` in `app/services/llm.py`), and the prompt instructs the LLM to split them into three roughly equal tiers:
   ...
   This is already the default behavior — nothing extra to pass. `send_to_telegram.py` already sends the full list twice: with `#` appended to the caption (`build_caption_with_hashtags`) and as plain comma-separated words for YouTube Studio's separate "Tags" upload field (`format_tags_field`). See `docs/skill/shorts_growth_guide.md` Rank 7 for the sourcing and `channel_playbook.md` §5 for when/why this was implemented (it used to emit only 3 flat, untiered hashtags — do not regress back to that).
