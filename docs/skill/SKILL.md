...
8j. **The OAuth refresh token in `docs/skill/youtube/token.json` can expire every ~7 days, not "never" as `authorize_local.py`'s docstring claims.** That claim is only true once the Google Cloud project's OAuth consent screen is verified/published; while it's still in "Testing" status, Google expires refresh tokens for test users after 7 days regardless of use. Hit this for real 2026-08-10 16:30: the 16:30 upload failed with `invalid_grant` / "Token has been expired or revoked" from `oauth2.googleapis.com/token`, and `token.json`'s file mtime was exactly 7 days earlier (created 2026-08-03). **Diagnosis, not guesswork** — reproduce the failing token refresh directly (`requests.post` to the token URI with the same params `upload_video.py` uses) and read the JSON error body (`error`/`error_description`), don't just trust the traceback's `HTTPError`. **Fix requires the owner**: re-run `authorize_local.py` on their own machine (this session has no browser for Google's consent screen) and send back a fresh `token.json`. Longer-term fix to stop this recurring: publish/verify the OAuth consent screen in Google Cloud Console. **On this failure: do not retry blindly, do not skip silently** — tell the owner exactly what failed and what they need to do via Telegram, leave the slot's `approved`/`published` state untouched in `storage/todays_uploads.json` (the video is still fine, still approved, just not uploaded), and log the failure in `episode_log.csv` so the next job (or the owner) has full context without re-diagnosing.
...
**Transition SFX is opt-in, OFF by default** since 2026-08-08 (`channel_playbook.md` §5e) — pass `--sfx` to add a synthesised whoosh at the hook→fact-1 cut and every fact boundary after it. It was on by default for one day (2026-08-07); after two real fix attempts the same day (a 4-variant pool instead of one fixed file, and starting the sound ~40% of its own duration *before* the cut so it bridges the outgoing/incoming shot instead of stinging the new one — both still in `app/services/viral.py`'s `add_transition_sfx`, `DEFAULT_SFX_BRIDGE_FRAC`) the owner said it still didn't sound good and asked to drop it. The code is not deleted, just no longer wired to run unless asked — same treatment as background music. `--sfx-volume` (default `0.35`) and `--sfx-file` (default the 4-variant pool directory `resource/sfx/transitions/`) still apply when `--sfx` is passed.

**Background music is opt-in, not default** — pass `--bgm` to add it (owner 2026-08-07: don't put music on every video). When used: `--bgm-volume` (default `0.12`, measured against ~-20 LUFS source music — **re-measure if the track library changes**). **`--bgm-file` defaults to a random pick from `resource/songs/` (29 tracks) on every render, not one fixed track**, since 2026-08-08 (owner: don't stay fixed on the default music, vary it per video) — pass `--bgm-file` explicitly to force one specific track, e.g. for a deliberate mood match. Both passes write their own intermediate file in the task dir (`with-captions.mp4` → `with-sfx.mp4` → `final-viral.mp4`) so a bad volume can be re-mixed from the right stage without re-rendering the base video.

**`story_episode.py` also supports `--bgm`/`--bgm-file`/`--bgm-volume`** since 2026-08-08 — same defaults and behavior as above (opt-in, random track per render). It has no transition SFX (the story format has no per-fact boundaries to hang a whoosh on) and never has. Before this date the story pipeline had no music support at all; adding it after the fact required a manual one-off script.
...
3. **Close on a direct but content-specific CTA** (see Shorts Virality Guidelines item 5 for the full rule). Asking for a subscribe or comment is allowed and is the strongest conversion moment in the video — what fails is a generic, reused line. Write a new one per episode, tie it to that episode's content, and rotate SUBSCRIBE / COMMENT / BOTH. **Say "subscribe," not "follow"** (updated 2026-08-27, per owner-shared YouTube Analytics showing 99.4% of watch time comes from non-subscribed viewers) — YouTube's actual action button reads "Subscribe," and "follow" is ambiguous with other platforms' terminology; matching the platform's own word removes a small but real bit of friction. Keep it to one line: a long outro with no reason to stay is a documented drop-off cause.
